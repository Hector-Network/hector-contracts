...
contract HectorBondNoTreasuryBNBDepository is Ownable {
    ...
    function deposit(
        ...
        uint256 priceInUSD = bondPriceInUSD().mul(10000 - discount).div(10000); // Stored in bond info
        uint256 nativePrice = _bondPrice().mul(10000 - discount).div(10000);
        ...
        uint256 payout = payoutFor(value, discount); // payout to bonder is computed
     ...
     *  @return uint
     */
    function payoutFor(uint256 _value, uint256 _discount)
        public
        view
        returns (uint256)
    {
        uint256 nativePrice = bondPrice().mul(10000 - _discount).div(10000);
