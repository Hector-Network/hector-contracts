...
contract HectorSubscriptionFactory is Ownable {
    /* ======== STORAGE ======== */
    ...
    function setHectorSubscriptionLogic(address _hectorSubscriptionLogic)
        external
        onlyOwner
    {
    ...
    function syncSubscriptions(
        ...
        for (uint256 i = 0; i < length; i++) {
            HectorSubscription(subscriptionContracts[i]).syncSubscriptions(
                froms[i]
            );
        }
    }
