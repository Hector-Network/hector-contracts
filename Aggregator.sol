...
contract Aggregator is Ownable {
    ...
    function add11v2BondContracts(address[] memory _contracts) external onlyPolicy() {
        for(uint i=0;i<_contracts.length;i++) {
            add11v2BondContract(_contracts[i]);
        }
    }

    function add11v2BondContract(address _contract) internal onlyPolicy() {
        require(_contract != address(0));
