...
async function main() {
	...
	const originCBridgeData = originSteps.includedSteps.find(
		(element: any) => element.type == 'cross'
	).estimate.data;
	...
	const mockBridgeData1 = {
		...
		sendingAssetId: enableSwap
			? enableSwap && originSwapData.estimate.data.contractAddress
				? originSteps.estimate.data.destToken
				: originSteps.estimate.data.buyTokenAddress
			: tempData.fromToken.address,
		receiver: tempData.toAddress,
		minAmount: enableSwap
			? enableSwap && originSwapData.estimate.data.contractAddress
				? originSteps.estimate.data.destAmount
				: originSwapData.estimate.toAmountMin
			: tempData.fromToken.address,
			...
			: [
					{
						...
						sendingAssetId:
							(enableSwap && originSwapData.estimate.data.sellTokenAddress == ETH_ADDRESS) ||
							isNativeFrom
								? ZERO_ADDRESS
