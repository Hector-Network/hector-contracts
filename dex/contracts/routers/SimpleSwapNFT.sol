...
abstract contract SimpleSwapNFT is FeeModel, IRouterNFT {
    ...
    function performSimpleBuyNFT(
        ...
        for (uint256 i = 0; i < toTokenDetails.length; i++) {
            ...
            if ((details.toToken & (1 << 160)) == 0) {
                UtilsNFT.transferTokens721(address(uint160(details.toToken)), beneficiary, details.toTokenID);
            } else {
                UtilsNFT.transferTokens1155(address(uint160(details.toToken)), beneficiary, details.toTokenID, details.toAmount);
            }
        }
