...
contract HectorRFQ is EIP712("HECTOR RFQ", "1") {
    ...
    bytes32 public constant RFQ_LIMIT_ORDER_TYPEHASH =
        keccak256(
    ...
    bytes32 public constant RFQ_LIMIT_NFT_ORDER_TYPEHASH =
        keccak256(
    ...
    */
    function partialFillOrderWithTarget(
        ...
        uint256 takerTokenFillAmount,
        address target
    ) public returns (uint256 makerTokenFilledAmount) {
        require(
            takerTokenFillAmount > 0 &&
                takerTokenFillAmount <= order.takerAmount,
            "Invalid Taker amount"
        );
        makerTokenFilledAmount =
            (takerTokenFillAmount * order.makerAmount) /
            order.takerAmount;
        require(
            makerTokenFilledAmount > 0,
            "Maker token fill amount cannot be 0"
        );
        _fillOrder(
            order,
            signature,
    ...
    */
    function fillOrder(Order calldata order, bytes calldata signature)
        external
    {
        fillOrderWithTarget(order, signature, msg.sender);
    }
    ...
    /**  
     ...
     @param signature Signature of the maker corresponding to the order
    */
    function fillOrderNFT(OrderNFT calldata order, bytes calldata signature)
        external
    {
        fillOrderWithTargetNFT(order, signature, msg.sender);
    }
    ...
    */
    function fillOrderWithTarget(
        ...
        require(
            takerTokenFillAmount > 0 && makerTokenFillAmount > 0,
            "Invalid amount"
        );
    ...
    */
    function batchFillOrderWithTarget(
        ...
        for (uint256 i = 0; i < orderInfos.length; i++) {
            ...
            require(
                takerTokenFillAmountOrder > 0 &&
                    takerTokenFillAmountOrder <= orderInfo.order.takerAmount,
                "Invalid Taker amount"
            );

            uint256 makerTokenFillAmountOrder = (takerTokenFillAmountOrder *
                orderInfo.order.makerAmount) / orderInfo.order.takerAmount;
            require(
                makerTokenFillAmountOrder > 0,
                "Maker token fill amount cannot be 0"
            );
    ...
    */
    function tryBatchFillOrderTakerAmount(
        ...
        for (uint256 i = 0; i < orderInfos.length; i++) {
            OrderInfo calldata orderInfo = orderInfos[i];
            uint256 takerFillAmountOrder = takerFillAmount >
                orderInfo.takerTokenFillAmount
                ? orderInfo.takerTokenFillAmount
                : takerFillAmount;
            ...
            if (success) takerFillAmount -= takerFillAmountOrder;

            if (takerFillAmount == 0) break;
        }
        require(
            takerFillAmount == 0,
            "Couldn't swap the requested fill amount"
        );
    }
    ...
    */
    function tryBatchFillOrderMakerAmount(
        ...
        for (uint256 i = 0; i < orderInfos.length; i++) {
            ...
            uint256 maxMakerFillAmount = (orderInfo.takerTokenFillAmount *
                orderMakerAmount) / orderTakerAmount;
            uint256 makerFillAmountOrder = makerFillAmount > maxMakerFillAmount
                ? maxMakerFillAmount
                : makerFillAmount;
            uint256 takerFillAmountOrder = ((makerFillAmountOrder *
                orderTakerAmount) + (orderMakerAmount - 1)) / orderMakerAmount;
            ...
            if (success) makerFillAmount -= makerFillAmountOrder;

            if (makerFillAmount == 0) break;
        }
        require(
            makerFillAmount == 0,
            "Couldn't swap the requested fill amount"
    ...
    */
    function batchFillOrderWithTargetNFT(
        ...
        for (uint256 i = 0; i < orderInfos.length; i++) {
            ...
            _permit(
                address(uint160(orderInfo.order.takerAsset)),
                orderInfo.permitTakerAsset
            );
            _permit(
                address(uint160(orderInfo.order.makerAsset)),
                orderInfo.permitMakerAsset
            );
    ...
    function _fillOrderNFT(
        ...
        address maker = order.maker;
        bytes32 orderHash = _hashTypedDataV4(
            keccak256(abi.encode(RFQ_LIMIT_NFT_ORDER_TYPEHASH, order))
        );
        _checkOrder(
            maker,
            order.taker,
     ...
     */
    function _cancelOrder(address maker, bytes32 orderHash)
        internal
        returns (bool)
    {
        mapping(bytes32 => uint256) storage remainingMaker = remaining[maker];
    ...
    function transferTokensNFT(
        ...
        if (tokenType == 0) {
            IERC20(address(uint160(token))).safeTransferFrom(from, to, amount);
        } else if (tokenType == 1) {
            ...
            IERC721(address(uint160(token))).safeTransferFrom(from, to, id);
        } else {
