...
contract DystopiaExchangeRouter {
    ...
    function _swap(
        ...
        if (tokenIn == ETH_IDENTIFIER) {
            ...
            TransferHelper.safeTransferFrom(tokenIn, msg.sender, address(uint160(pools[0])), amountIn);
        ...
        for (uint256 i = 0; i < pairs; ++i) {
            ...
            IDystPair(pool).swap(
                amount0Out,
                amount1Out,
                i + 1 == pairs ? (tokensBoughtEth ? address(this) : msg.sender) : address(uint160(pools[i + 1])),
                ""
            );
        }
