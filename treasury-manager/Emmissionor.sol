...
interface IRewardWeight{
    ...
    function getRewardAmount(address receiver) external returns(uint);
}
...
contract Emissionor is Ownable {
    ...
    uint totalRewardMinted;     //Weekly reward amount adjusted manually or from a contract
    uint totalSentToSplitter; //Tracking rewards sent to splitter contract
    ...
    constructor(address _treasury, address _splitter, address  _rewardToken) {
    ...
    function sendRewards() external onlyOwner {
        require(totalRewardMinted > 0, "Need to allocate reward for distribution");

        //mint rewards from treasury
        ITreasury( treasury ).mintRewards(address(this), totalRewardMinted);

        require(IERC20(rewardToken).balanceOf(address(this)) > 0, "No reward to distribute");
        ...
        emit RewardsDistributed( msg.sender, address(splitter), totalRewardMinted );
   ...
   /* ====== VIEW FUNCTIONS ====== */
    ...
    function setRewardAmountByContract(address receiver) external onlyOwner {
        ...
        uint amount = IRewardWeight(rewardWeightContract).getRewardAmount(receiver);
