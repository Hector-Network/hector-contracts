...
async function main() {
	...
	const votingContract = new ethers.Contract(VOTING_ADDRESS, abi.abi, deployer);
	...
	const fnftVotingInfoFromHistories: any = [];

	const getVotingMethodDecode = (voteTx: any) => {
	...
	txHistories.map((history: any) => {
		if (history.txreceipt_status && history.txreceipt_status != '0') {
			...
			const newWeights: Array<BigNumber> = [];
			weights.value.map((weight: string) => {
				newWeights.push(BigNumber.from(weight));
			});
			const newFNFTIds: any = [];
			fnftIds.value.map((fnftId: string) => {
