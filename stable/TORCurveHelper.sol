...
contract TORCurveHelper is ITORCurveHelper{
    ...
    function getTorAndStable() view public returns(uint torAmount,uint stableAmount){//1e18 for torAmount and stableAmount
        ...
        stableAmount=c2pool.balances(1).mul(1e12).mul(c2Amount).div(c2pool.totalSupply()).add(c2pool.balances(0).mul(c2Amount).div(c2pool.totalSupply()));
    }
    function getPoolPercentageWithMint(uint _torAmount) override view external returns(uint percentage){//tor percentage 10000 = 100%
        (uint torAmount,uint stableAmount)=getTorAndStable();
        percentage=torAmount.add(_torAmount).mul(1e4).div(torAmount.add(stableAmount));
    }
    function getPoolPercentageWithRedeem(uint _torAmount) override view external returns(uint percentage){//tor percentage 5000 = 50%
        (uint torAmount,uint stableAmount)=getTorAndStable();
        percentage=torAmount.sub(_torAmount).mul(1e4).div(torAmount.add(stableAmount));
    }
}
