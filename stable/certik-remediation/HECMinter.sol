...
contract HECMinter is IHECMinter,Ownable{
    ...
    function setHec(address _HEC) external onlyOwner(){
        require(_HEC!=address(0), "set HEC to the zero address");
        HEC=_HEC;
    }
    ...
    function mintHEC(uint amount) override external{
        require(msg.sender==TORMinter,"only TOR minter can mint HEC this way");
    ...
    function burnHEC(uint amount) override external{
        require(msg.sender==TORMinter,"only TOR minter can burn HEC this way");
        require(amount>0,"invalid amount");
    ...
    function burnFrom(address account, uint256 amount) internal{
        require(IERC20(HEC).transferFrom(account,address(this),amount), 'Unsuccessful transfer');
