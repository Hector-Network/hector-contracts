...
contract BondHelper is Ownable {
    ...
    mapping(address=>uint) public bondMap;//store sequence of bonds,sequence=index+1
    ...
    function deposit( address _bond, uint _amount,uint _maxPrice) external returns (uint){
    ...
    function removeApprovalForAllBonds() external {
        for( uint i = 0; i < bonds.length; i++ ) {
            if ( bonds[i] != address(0) ) {
                if ( IERC20(IBond( bonds[i] ).principle()).allowance(msg.sender, bonds[i]) > 0 ) {
                    IERC20(IBond( bonds[i] ).principle()).approve(bonds[i],0);
                }
            }
        }
    }

    function hasApprovalOnAnyBond() external view returns (bool) {
        for( uint i = 0; i < bonds.length; i++ ) {
            if ( bonds[i] != address(0) ) {
                if ( IERC20(IBond( bonds[i] ).principle()).allowance(msg.sender, bonds[i]) > 0 ) {
                    return true;
    ...
    function removeBondContract( address _bond, uint _sequence ) external onlyPolicy() {
        require(_sequence>0,"invalid sequence");
        require(bondMap[_bond]!=0,"bond not registered");
        uint index=_sequence.sub(1);
